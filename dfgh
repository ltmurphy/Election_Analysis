...
with open(file_to_save, "w") as txt_file:
    ...
    for county_name in county_votes:
        ...
        county_vote_results = (f"{county_name}: {county_vote_percentage:.1f}% ({county_vote:,})\n")
        print(county_vote_results)
         # 6e: Save the county votes to a text file.
        txt_file.write(county_vote_results)
         # 6f: Write an if statement to determine the winning county and get its vote count.
        if (county_vote > votes_largest_turnout) and (county_vote_percentage > county_win_percentage):
